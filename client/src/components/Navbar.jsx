  ...
  return (
    <motion.nav
      ...
      className={`fixed left-0 right-0 mx-auto z-50 transition-all duration-300 ${
        isMobileMenuOpen 
        ? 'top-4 w-[95%] bg-gunmetal/95 backdrop-blur-xl rounded-[2rem] shadow-2xl overflow-hidden' // Minimal expanded card
        : `top-4 w-[95%] md:w-[85%] max-w-6xl rounded-full ${isScrolled ? 'bg-gunmetal/80 backdrop-blur-xl shadow-2xl border border-white/10 py-2 px-2' : 'bg-gunmetal shadow-xl py-3 px-6'}`
      }`}
    >
      <div className={`w-full flex items-center justify-between ${isMobileMenuOpen ? 'p-4 pb-0' : ''}`}>
        ...
        <a href="#" className="flex items-center gap-2 group pl-2">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-primary to-accent flex items-center justify-center text-white font-bold text-sm shadow-lg group-hover:rotate-12 transition-transform">
             P
          </div>
      ...
      {/* Mobile Menu Content - Clean Dropdown List */}
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden w-full flex flex-col items-center justify-center gap-2 py-8"
          >
              {navLinks.map((link, index) => (
                <motion.a
                  key={link.name}
